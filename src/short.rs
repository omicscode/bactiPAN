...
fn run_macse(dirpath: &str, threads: usize) -> io::Result<()> {
    ...
    let paths: Vec<_> = glob(&format!("{}/*.fa", dirpath))
        .unwrap()
    ...
    let aligned_paths: Vec<_> = glob("*.ntaligned.fasta")
        .unwrap()
        .collect::<Result<Vec<_>, _>>()
        .unwrap();
...
fn run_prank(dirpath: &str, threads: usize) -> io::Result<()> {
    ...
    let aligned_paths: Vec<_> = glob("*.prankaligned.fasta")
        .unwrap()
        .collect::<Result<Vec<_>, _>>()
        .unwrap();
    aligned_paths.par_iter().try_for_each(|aligned_path| {
...
fn run_muscle(dirpath: &str, threads: usize) -> io::Result<()> {
    ...
    let aligned_paths: Vec<_> = glob("*.musclealigned.fasta")
        .unwrap()
        .collect::<Result<Vec<_>, _>>()
        .unwrap();
    aligned_paths.par_iter().try_for_each(|aligned_path| {
